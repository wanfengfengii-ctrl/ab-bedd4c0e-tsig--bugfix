...
- 轮换是原子的：安装新 ACTIVE 时旧 ACTIVE 变为带明确 `expiresAt` 的
  RETIRING；可立即撤销。
- **首末消息签名**，且每满 100 条至少再签一次（RFC 2845 §4.4 / RFC 8945
  §5.3.2：连续未签名消息不超过 99 条）；多消息响应维持**同一条连续
  running-MAC 认证链**（请求 MAC → 每条消息逐一推进 → 末响 MAC），每条
  未签名中间消息都计入下一条签名，任一字节变化都会改变后续签名。
- 请求与响应的 MAC 输入均为**移除 TSIG 后、ARCOUNT 未加一**的报文；响应
  计算中前序 MAC 以两字节长度前缀接入（RFC 2845 §4.2/§4.3）。
