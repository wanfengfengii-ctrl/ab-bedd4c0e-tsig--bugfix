...
- 轮换是原子的：安装新 ACTIVE 时旧 ACTIVE 变为带明确 `expiresAt` 的
  RETIRING；可立即撤销。
- **首条、末条以及每隔不超过 100 条（RFC 2845 §4.4）的周期性消息均签名**，
  其余中间消息不签名；多消息响应在**同一条连续 HMAC 状态**上推进 running-MAC
  （请求 MAC → 首响 MAC → 周期/末响 MAC）：每条未签名中间消息的原始线格式
  都被喂入当前 MAC 上下文，故其中任意字节变化都会使后续签名改变；首条按
  §4.2 加入完整 TSIG 变量，后续签名消息只追加 TSIG 计时器（time||fudge）。
  单消息传送即“首条=末条”，携带一个可验证 TSIG。
